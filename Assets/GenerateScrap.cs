...
public class GenerateScrap : MonoBehaviour
{
    ...
    GameObject scrap; // Scrap is used to buy cards in the scrapyard 
    GameObject coin; // Coins are used to purchace items from
    GameObject ration; // Rations are used for healing
    GameObject fuel; // Fuel is used to travel to the next level
    ...
    public void RandomItem(int modifier, int counter)
    {
        ...
        while (counters > 0)
        {
            if (totalCount < 0) GenerateSpecific(scrap);
            else
            {
                totalCount -= rationCount;
                if (totalCount < 0) Instantiate(ration, transform.position, transform.rotation);
                else
                {
                    totalCount -= fuelCount;
                    if (totalCount < 0) Instantiate(fuel, transform.position, transform.rotation);
                    else
                    {
                        totalCount -= levelKeyCount;
                        if (totalCount < 0) Instantiate(levelKey, transform.position, transform.rotation);
                        else Instantiate(coin, transform.position, transform.rotation);
                    }
                }
            }
            counters--;
