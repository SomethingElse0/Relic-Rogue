...
public class Enemy2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Time.fixedTime < attackEndTime) rb.linearVelocity = rb.linearVelocity.normalized * 8;
        else if(attacking == true)
        {
            ...
            agent.enabled = true;
            attacking = false;
            attackEndPosition = transform.position;
        }
        else
        {
            if (transform.localPosition.magnitude>0.1f)
            {
                bool isWarped =agent.Warp(new Vector3(transform.position.x, transform.position.y, transform.parent.position.z));
                if (!isWarped) print("STOP");
    ...
    void Attack()
    {
        ...
        rb.linearVelocity = (player.transform.position-transform.position).normalized * 8;
